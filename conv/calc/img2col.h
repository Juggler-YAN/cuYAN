...
void img2col(const float* x, const float* w, float* y, const int N, const int IN_C, const int IN_H,
...
#define IY(n, out_c, out_h, out_w) ((((n) * OUT_C + out_c) * OUT_H + out_h) * OUT_W + out_w)
    ...
    const int NEW_K_H = (K_H - 1) * (DILATION_H - 1) + K_H;
    const int NEW_K_W = (K_W - 1) * (DILATION_W - 1) + K_W;

    for (int n = 0; n < N; ++n) {
        // (1,Cin,Hin,Win) * (Cout,Cin,Hk,Wk) = (1,Cout,Hout,Wout)
        float *A = (float *)malloc((OUT_H * OUT_W) * (IN_C * NEW_K_H * NEW_K_W) * sizeof(float));
        float *B = (float *)malloc((IN_C * NEW_K_H * NEW_K_W) * OUT_C * sizeof(float));
        float *C = (float *)malloc(OUT_C * (OUT_H * OUT_W) * sizeof(float));
        memset(A, 0, (OUT_H * OUT_W) * (IN_C * NEW_K_H * NEW_K_W) * sizeof(float));
        memset(B, 0, (IN_C * NEW_K_H * NEW_K_W) * OUT_C * sizeof(float));
                    ...
                    for (int k_h = 0; k_h < NEW_K_H; k_h += DILATION_H) {
                        for (int k_w = 0; k_w < NEW_K_W; k_w += DILATION_W) {
                            ...
                            if (real_in_h >= 0 && real_in_h < IN_H && real_in_w >= 0 && real_in_w < IN_W) {
                                int pos = (out_h * OUT_W + out_w) * (IN_C * NEW_K_H * NEW_K_W) + 
                                    in_c * NEW_K_H * NEW_K_W + k_h * NEW_K_W + k_w;
                                A[pos] = x[IX(n, in_c, real_in_h, real_in_w)];
                            }
                        }
                    }
                }
            ...
            for (int in_c = 0; in_c < IN_C; ++in_c) {
                for (int k_h = 0; k_h < NEW_K_H; k_h += DILATION_H) {
                    for (int k_w = 0; k_w < NEW_K_W; k_w += DILATION_W) {
                        ...
                        int pos = (in_c * NEW_K_H * NEW_K_W + k_h * NEW_K_W + k_w) * OUT_C + out_c;
                        B[pos] = w[IW(out_c, in_c, real_k_h, real_k_w)];
                    }
                }
            }
        }
        ...
        for (int out_c = 0; out_c < OUT_C; ++out_c) {
            for (int out_h = 0; out_h < OUT_H; ++out_h) {
                for (int out_w = 0; out_w < OUT_W; ++out_w) {
                    float temp = 0.0f;
                    for (int k = 0; k < IN_C * NEW_K_H * NEW_K_W; ++k) {
                        temp += (float)A[(out_h * OUT_W + out_w) * (IN_C * NEW_K_H * NEW_K_W) + k] * 
